...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("laska.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku laska.tga");
		exit(0);
	}
	...
	error = load_targa("laska.tga", width, height, format, type, pixels);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("laska.tga (kompresja)", TEXTURE_LENA);
	glutAddMenuEntry("laska.tga (bez kompresji)", TEXTURE_LENA_UNC);
	glutAddMenuEntry("laska.tga (kompresja)", TEXTURE_LENA_GRAY);
	glutAddMenuEntry("laska.tga (bez kompresji)", TEXTURE_LENA_GRAY_UNC);
