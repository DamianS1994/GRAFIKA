...
void initBezier(void) {
	...
	mybezier.anchors[0][0] = makePoint(-0.75 + xx, -0.75 + xx, -0.50 + xx);

	mybezier.anchors[0][1] = makePoint(-0.25, -0.75 + y, 0.00);

	//mybezier.anchors[0][2] = makePoint(0.25, -0.75, 0.00);

	//mybezier.anchors[0][3] = makePoint(0.75, -0.75, -0.50);

	mybezier.anchors[1][0] = makePoint(-0.75, -0.25, -0.75 + z);
...
int InitGL(GLvoid)
{
	...
	if (texture == MARS_TEX)
	{
		error = load_targa("saturn.tga", width_texture, height_texture, format, type, Ptr_mybezier_texture);
		if (error == GL_FALSE)
		{
	...
	{
		error = load_targa("woda.tga", width_texture, height_texture, format, type, Ptr_mybezier_texture);
		if (error == GL_FALSE)
		{
			printf("Niepoprawny odczyt pliku");
			exit(0);
...
void DrawGLScene(GLvoid) {
	...
	mybezier.anchors[0][0] = makePoint(-0.75 + iSecret, -0.75 + iSecret, -0.50 + iSecret);

	mybezier.anchors[0][1] = makePoint(-0.25 + iSecret, -0.75 + y, 0.00);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("Metal", STONE_TEX);
